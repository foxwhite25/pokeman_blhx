...
try:
    ...
    unknown_chara_icon = R.img(f'blhx/unit/icon_unit_{UNKNOWN}31.png').open()
        ...
        for idx, names in _blhx_data.CHARA_NAME.items():
            for n in names:
                ...
                if n not in self._roster:
                    ...
                    logger.warning(f'blhx.chara.Roster: 出现重名{n}于id{idx}与id{self._roster[n]}')
...
class Chara:
    ...
    @property
    def name(self):
        return _blhx_data.CHARA_NAME[self.id][0] if self.id in _blhx_data.CHARA_NAME else \
        _blhx_data.CHARA_NAME[UNKNOWN][0]
