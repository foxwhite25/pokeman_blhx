...
MIX_PROBABILITY = {str(list((-1, -1))): [0.8, 0.194, 0.006], str(list((-1, 0))): [0.44, 0.5, 0.06],
                   str(list((-1, 1))): [0.55, 0.3, 0.1],
                   str(list((0, 0))): [0.1, 0.8, 0.1], str(list((0, 1))): [0.3, 0.5, 0.2],
                   str(list((1, 1))): [0.15, 0.25, 0.6]}
# 一键合成概率
OK_MIX_PROBABILITY = {str(list((-1, -1))): [0.846, 0.15, 0.004], str(list((-1, 0))): [0.56, 0.4, 0.04],
                      str(list((-1, 1))): [0.68, 0.24, 0.08],
                      str(list((0, 0))): [0.33, 0.6, 0.07], str(list((0, 1))): [0.44, 0.4, 0.16],
...
def get_chara_name(card_id):
    ...
    if 3000 > chara_id > 2000:
        ...
        chara_id += 1000
        rarity_desc = '【普通】的'
    return rarity_desc, chara_blhx.fromid(chara_id).name
...
@sv.on_prefix('碧蓝航线抽卡')
async def word_back_(bot, ev: CQEvent):
    gid = ev['group_id']
    uid = ev['user_id']
    ...
    if not daily_limiter.check(guid) or random.random() > POKE_GET_CARDS:
        ...
        card_counter, card_descs, card = get_random_cards(db.get_cards_num(gid, uid), row_num, col_num,
                                                          amount, True, get_random_cards_list, SUPER_RARE_PROBABILITY,
                                                          RARE_PROBABILITY)
        dash = '----------------------------------------'
        msg_part = '\n'.join(card_descs)
        ...
        for card_id in card_counter.keys():
            db.add_card_num(
                gid, uid, card_id, card_counter[card_id])
        daily_limiter.increase(guid)


@sv.on_prefix('碧蓝航线一键抽卡')
...
@sv.on_prefix('碧蓝航线群排行榜')
async def get_rank_(bot, ev: CQEvent):
    gid = ev['group_id']
    uid = ev['user_id']
    r = db.get_group_rank(gid)
    msg = "群排行榜:\n"
    done = False
    ...
    for rank in r:
        if realrank < 21:
            ranknow = db.get_group_ranking(gid, rank[0])
            msg += f"第{ranknow}名:[CQ:at,qq={rank[0]}],持有{rank[1]}张不同卡片\n"
            realrank += 1
        elif not done == True:
            msg += f'...\n'
            msg += f"你当前群排名: {ranking_desc}"
            done = True
    await bot.send(ev, msg)


@sv.on_prefix(('碧蓝航线献祭', '碧蓝航线合成', '碧蓝航线融合'))
async def mix_card_(bot, ev: CQEvent):
    # 参数识别
...
@sv.on_prefix(('碧蓝航线一键献祭', '碧蓝航线一键合成', '碧蓝航线一键融合', '碧蓝航线全部献祭', '碧蓝航线全部合成', '碧蓝航线全部融合'))
async def auto_mix_card_(bot, ev: CQEvent):
    # 参数识别
    s = ev.message.extract_plain_text()
    args = s.split(' ')
    if len(args) == 2 and args[0] in rarity_desc2rarity and args[1] in rarity_desc2rarity:
        pass
    elif len(args) == 3 and args[0] in rarity_desc2rarity and args[1] in rarity_desc2rarity and args[
        2].isdigit() and int(args[2]) > 0:
        pass
    else:
    ...
    card_counter, card_descs, card = get_random_cards(db.get_cards_num(
        ev.group_id, ev.user_id), row_num, col_num, mix_rounds, False, get_random_cards_list, super_rare_prob,
        rare_prob)
    msg_part = '\n'.join(card_descs)
...
@sv.on_prefix(('碧蓝航线交换', '碧蓝航线交易', '碧蓝航线互换'))
async def exchange_cards_(bot, ev: CQEvent):
    # 参数识别
    if len(ev.message) != 3:
...
async def confirm_exchange_(bot, ev: CQEvent):
    ...
    if card1_num == 0:
        await bot.finish(ev,
                         f'{MessageSegment.at(exchange_request.sender_uid)}的{get_card_name_with_rarity(exchange_request.card1_name)}卡数量不足, 无法交换')
    if card2_num == 0:
        await bot.finish(ev,
                         f'{MessageSegment.at(exchange_request.target_uid)}的{get_card_name_with_rarity(exchange_request.card2_name)}卡数量不足, 无法交换')
...
@sv.on_prefix(('碧蓝航线赠送', '碧蓝航线白给', '碧蓝航线白送'))
async def give_(bot, ev: CQEvent):
    ...
    await bot.send(ev,
                   f'{MessageSegment.at(ev.user_id)}将{get_card_name_with_rarity(card_name)}赠送给了{MessageSegment.at(target_uid)}')


@sv.on_prefix('碧蓝航线查看仓库')
async def storage_(bot, ev: CQEvent):
...
@sv.on_fullmatch('碧蓝航线刷新卡片')
async def refresh_unit_cache_(bot, event: CQEvent):
    cards['1'] = []
    cards['3'] = []
    cards['6'] = []
